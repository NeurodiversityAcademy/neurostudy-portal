...
describe('CourseDetailsBodyText', () => {
  ...
  it('renders tuition fees button only for tuitionFees id', () => {
    render(
      <CourseDetailsBodyText id='tuitionFees' data={'Fee details here'} />,
    );
  ...
  it('does not render tuition fees button for other ids', () => {
    render(
      <CourseDetailsBodyText id='overview' data={'Overview text'} />,
    );

    expect(
      screen.queryByRole('button', { name: /view tuition fees/i }),
    ).not.toBeInTheDocument();
  });

  it('handles undefined data gracefully', () => {
    const { container } = render(
      <CourseDetailsBodyText id='overview' data={undefined} />,
    );
