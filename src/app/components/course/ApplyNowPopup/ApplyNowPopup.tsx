...
export default function ApplyNowPopup({ open, onClose }: ApplyNowPopupProps) {
  const methods: UseFormReturn<ApplyNowFieldValues> = useForm<ApplyNowFieldValues>({
    mode: 'onBlur',
  });
      ...
      <div className={styles.container}>
        ...
        <Form methods={methods} onSubmit={methods.handleSubmit(onSubmit)} className={styles.form}>
          ...
          <Typography variant={TypographyVariant.Body3} className='mb-3'>
            Fill in a few details and we&apos;ll get back to you about next steps.
          </Typography>

          <TextBox name='full_name' label='Full name' required placeholder='Your full name' />
          <TextBox
            name='number'
            label='Number (optional)'
            placeholder='Reference / student number'
          />
          <TextBox name='phone' label='Phone number' required placeholder='Phone number' />
          <TextBox
            name='email'
            type='email'
            label='Email Address'
          ...
          {isSubmitted && (
            <Typography variant={TypographyVariant.Body2} className={styles.fileReadyText}>
              Thanks — your application has been received. We&apos;ll contact you soon.
            </Typography>
          )}
