...
import { GOAL_FIELDS, PROFILE_FIELD_OPTIONS } from '@/app/utilities/profile/constants';
...
> = forwardRef<ProfileSectionRef, ProfileSectionFormProps>(({ onSubmit, onCancel }, ref) => {
  ...
  return (
    <Form methods={methods} onSubmit={onSubmit ? methods.handleSubmit(onSubmit) : emptyFunc}>
      <Dropdown<UserGoalProps>
        ...
        rules={{
          validate: {
            limit3: (value) => (value?.length || 0) <= 3 || 'Choose at most 3.',
          },
        }}
      />
      <Dropdown<UserGoalProps>
        ...
        rules={{
          validate: {
            limit5: (value) => (value?.length || 0) <= 5 || 'Choose at most 5.',
          },
        }}
      />
      <Dropdown<UserGoalProps>
