...
describe('useVisitedItems', () => {
  const mockSearchParams = new URLSearchParams() as unknown as
    import('next/navigation').ReadonlyURLSearchParams;
  ...
  it('returns an empty array when nothing is stored', () => {
    const { result } = renderHook(() =>
      useVisitedItems('article', mockSearchParams)
    );
  ...
  it('returns visited blog ids from localStorage', () => {
    ...
    const { result } = renderHook(() =>
      useVisitedItems('blog', mockSearchParams)
    );
