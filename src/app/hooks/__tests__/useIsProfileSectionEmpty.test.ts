...
describe('useIsProfileSectionEmpty', () => {
  it('returns truthy when data is undefined', () => {
    const { result } = renderHook(() => useIsProfileSectionEmpty(undefined));

    expect(result.current).toBeTruthy();
  });

  it('returns true for an empty object', () => {
    const { result } = renderHook(() => useIsProfileSectionEmpty({} as UserProps));
  ...
  it('returns true when all string values are empty', () => {
    ...
    const { result } = renderHook(() => useIsProfileSectionEmpty(data));
  ...
  it('caches the result when data does not change', () => {
    ...
    const { result, rerender } = renderHook(() => useIsProfileSectionEmpty(data));
