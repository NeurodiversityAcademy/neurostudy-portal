...
describe('isValidContactUsFormData', () => {
  ...
  it('returns true for valid data with valid phone', () => {
    expect(
      isValidContactUsFormData({ ...validData, phone: '0412345678' })
    ).toBe(true);
  });

  it('returns true for valid data with +61 phone format', () => {
    expect(
      isValidContactUsFormData({ ...validData, phone: '+61412345678' })
    ).toBe(true);
  });
  ...
  it('returns false for invalid email (no @)', () => {
    expect(
      isValidContactUsFormData({ ...validData, email: 'johnexample.com' })
    ).toBe(false);
  });

  it('returns false for invalid email (no domain)', () => {
    expect(
      isValidContactUsFormData({ ...validData, email: 'john@' })
    ).toBe(false);
  });

  it('returns false for invalid email (no TLD)', () => {
    expect(
      isValidContactUsFormData({ ...validData, email: 'john@example' })
    ).toBe(false);
  });

  it('returns false for invalid phone number', () => {
    expect(
      isValidContactUsFormData({ ...validData, phone: '123' })
    ).toBe(false);
  });

  it('returns false for phone starting with 0 or 1', () => {
    expect(
      isValidContactUsFormData({ ...validData, phone: '0112345678' })
    ).toBe(false);
  });

  it('returns true for valid email with subdomain TLD', () => {
    expect(
      isValidContactUsFormData({ ...validData, email: 'user@mail.co.uk' })
    ).toBe(true);
  });
});
...
describe('isValidTeacherRegistrationData', () => {
  ...
  it('returns false for invalid firstname with special chars', () => {
    expect(
      isValidTeacherRegistrationData({ ...validData, firstname: 'J@ne!' })
    ).toBe(false);
  });

  it('returns false for invalid lastname with special chars', () => {
    expect(
      isValidTeacherRegistrationData({ ...validData, lastname: 'Sm!th#' })
    ).toBe(false);
  });
  ...
  it('returns false for invalid email format', () => {
    expect(
      isValidTeacherRegistrationData({ ...validData, email: 'not-an-email' })
    ).toBe(false);
  });

  it('returns false for invalid phone (too short)', () => {
    expect(
      isValidTeacherRegistrationData({ ...validData, phone: '041234' })
    ).toBe(false);
  });

  it('returns false for invalid phone (starts with 1)', () => {
    expect(
      isValidTeacherRegistrationData({ ...validData, phone: '0112345678' })
    ).toBe(false);
  });

  it('returns true for phone with +61 prefix', () => {
    expect(
      isValidTeacherRegistrationData({ ...validData, phone: '+61298765432' })
    ).toBe(true);
  });
...
describe('isValidUserSubscriptionData', () => {
  it('returns true for valid email', () => {
    expect(isValidUserSubscriptionData({ email: 'user@example.com' })).toBe(
      true
    );
  });
  ...
  it('returns false for invalid email (no @)', () => {
    expect(isValidUserSubscriptionData({ email: 'userexample.com' })).toBe(
      false
    );
  });
  ...
  it('returns true for email with dots in local part', () => {
    expect(
      isValidUserSubscriptionData({ email: 'first.last@example.com' })
    ).toBe(true);
  });

  it('returns true for email with plus sign', () => {
    expect(
      isValidUserSubscriptionData({ email: 'user+tag@example.com' })
    ).toBe(true);
  });
});
...
describe('assertCourseData', () => {
  ...
  it('error has status 400', () => {
    expectAPIError(
      () => assertCourseData('bad' as unknown as Record<string, unknown>[])
    );
  });
});
...
describe('assertCourseWithoutIdData', () => {
  ...
  it('does not throw for valid array', () => {
    expect(() =>
      assertCourseWithoutIdData([validItem] as any)
    ).not.toThrow();
  });
  ...
  it('does not throw for subset of valid keys', () => {
    expect(() =>
      assertCourseWithoutIdData([{ Title: 'Only Title' }] as any)
    ).not.toThrow();
  });
});
