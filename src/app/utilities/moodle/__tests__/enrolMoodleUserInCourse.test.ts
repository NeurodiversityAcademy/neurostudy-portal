...
describe('enrolMoodleUserInCourse', () => {
  ...
  it('enrols a user in a course with the student role', async () => {
    ...
    expect(formData.get('enrolments[0][roleid]')).toBe(
      MOODLE_STUDENT_ROLE_ID.toString()
    );
  ...
  it('succeeds when moodle returns a null response body', async () => {
    ...
    await expect(
      enrolMoodleUserInCourse({ userid: 1, courseid: 2 })
  ...
  it('throws when moodle returns an exception payload', async () => {
    ...
    await expect(
      enrolMoodleUserInCourse({ userid: 1, courseid: 2 })
    ).rejects.toThrow('Failed to enrol the moodle user into the course.');
  });
